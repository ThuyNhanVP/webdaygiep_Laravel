            ...
            <tbody>
                @forelse($products as $sp)
                    <tr class="border-b hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <img src="{{ $sp->image_main }}" alt="Ảnh chính" class="w-16 h-16 object-cover rounded">
                        </td>
                        <td class="px-6 py-4">
                            <img src="{{ $sp->image_hover }}" alt="Ảnh hover" class="w-16 h-16 object-cover rounded">
                        </td>
                        <td class="px-6 py-4 text-center space-y-2">
                            ...
                            <div id="editModal-{{ $sp->id }}" 
                                ...
                                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
                                    ...
                                    <form action="{{ route('admin.products.update', $sp->id) }}" method="POST" class="space-y-3">
                                        @csrf
                                        ...
                                        <input type="number" name="price" value="{{ $sp->price }}" placeholder="Giá"
                                               class="w-full px-3 py-2 border rounded-md">
                                        <input type="text" name="category" value="{{ $sp->category }}" placeholder="Danh mục"
                                               class="w-full px-3 py-2 border rounded-md">
                                        <input type="text" name="colors" value="{{ $sp->colors }}" placeholder="Màu sắc"
                                               class="w-full px-3 py-2 border rounded-md">
                                        <input type="text" name="tag" value="{{ $sp->tag }}" placeholder="Tag"
                                               class="w-full px-3 py-2 border rounded-md">
                                        <input type="text" name="image_main" value="{{ $sp->image_main }}" placeholder="Ảnh chính"
                                               class="w-full px-3 py-2 border rounded-md">
                                        <input type="text" name="image_hover" value="{{ $sp->image_hover }}" placeholder="Ảnh hover"
                                               class="w-full px-3 py-2 border rounded-md">
                            ...
                            <form action="{{ route('admin.products.delete', $sp->id) }}" method="POST" class="inline"
                                ...
                                @csrf
                                @method('DELETE')
                                <button class="w-full px-3 py-1 bg-red-600 text-white text-xs rounded-lg hover:bg-red-700 transition">
                                    Xóa
                                </button>
                            </form>
                        </td>
